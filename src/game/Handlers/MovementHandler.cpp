...
void WorldSession::HandleMoverRelocation(Unit* pMover, MovementInfo& movementInfo)
{
    ...
    if (Player* pPlayerMover = pMover->ToPlayer())
    {
        ...
        if (movementInfo.HasMovementFlag(MOVEFLAG_ONTRANSPORT))
        {
            ...
            {
                if (Transport* t = pPlayerMover->GetMap()->GetTransport(pPlayerMover->m_movementInfo.GetTransportGuid()))
                {
            ...
            if (pPlayerMover->GetTransport()) 
            {//DMT
                pPlayerMover->m_movementInfo.pos.x = pPlayerMover->m_movementInfo.GetTransportPos()->x; //movementInfo.GetTransportPos().x
