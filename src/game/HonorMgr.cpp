...
void HonorMaintenancer::CheckMaintenanceDay()
{
    ...
    {
        // Restart 5 minutes after honor weekend by server time
        if (sWorld.getConfig(CONFIG_BOOL_AUTO_HONOR_RESTART))
        {
            sWorld.ShutdownServ(300, SHUTDOWN_MASK_RESTART, SHUTDOWN_EXIT_CODE);
            sLog.outString("HonorMaintenancer: Server will be restarted in 5 minutes to perform honor rank calculations.");
        }
        else
