...
void Map::SendInitSelf(Player* player)
{
    ...
    {
        hasTransport = true;
        DETAIL_LOG("attach to player data current transport data %u", player->GetGUIDLow());
        transport->BuildCreateUpdateBlockForPlayer(&data, player);
    }

    // build data for self presence in world at own client (one time for map)
    DETAIL_LOG("build data for self presence in world at own client (one time for map) %u", player->GetGUIDLow()); 
    player->BuildCreateUpdateBlockForPlayer(&data, player);

    // build other passengers at transport also (they always visible and marked as visible and will not send at visibility update at add to map
    DETAIL_LOG("build other passengers at transport %u", player->GetGUIDLow());
    if (Transport* transport = player->GetTransport())
    {
        DETAIL_LOG("if build other passengers at transport %u", player->GetGUIDLow());
    ...
    DETAIL_LOG("Ready data.Send %u", player->GetGUIDLow());
