...
void Player::AddStartingItems()
{
    ...
    if (!info)
    {
        sLog.outString( "Player have incorrect race/class pair. Can't add starting items.");
        return;
    }
...
void Player::_LoadSpellCooldowns(QueryResult* result)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (item_id)
            {
                itemProto = sObjectMgr.GetItemPrototype(item_id);
                if (!itemProto)
                {
...
void Player::SaveToDB(bool online, bool force)
{
    ...
    // Do not save bots DMT

    if (GetSession()->GetBot())
		if (GetSession()->GetBot()->accountId >= 10000)
			return;
