    ...
    for (const auto& spell : me->GetSpellMap())
    {
        ...
        switch (me->GetClass())
        {
            ...
            case CLASS_HUNTER:
            {
				...
				else if (pSpellEntry->SpellName[0].find("Tranquilizing Shot") != std::string::npos)
				{
					if (!m_spells.hunter.pTranquilizingShot ||
						m_spells.hunter.pTranquilizingShot->Id < pSpellEntry->Id)
            ...
            case CLASS_PRIEST:
            {
				...
				else if (pSpellEntry->Id == (6346)) //Fear Ward
				{
				if (!m_spells.priest.pFearWard ||
					m_spells.priest.pFearWard->Id < pSpellEntry->Id)
...
/*
SpellEntry const* CombatBotBaseAI::SelectBestBlessingSpell(Unit const* pTarget) const
{
    if (pBlessingOfSanctuary && m_role == ROLE_TANK)
        m_spells.paladin.pBlessingBuff = pBlessingOfSanctuary;
    else
...
bool CombatBotBaseAI::HealInjuredTargetDirect(Unit* pTarget)
{
    if (SpellEntry const* pHealSpell = SelectMostEfficientHealingSpell(pTarget, spellListDirectHeal))
		if (DoCastSpell(pTarget, pHealSpell) == SPELL_CAST_OK)
			return true;
			//char sMessage[200] = "";
			//sprintf(sMessage, "%s %s", pTarget->GetName(), sSpellMgr.mSpellEntryMap[pHealSpell->Id]->SpellName[4].c_str());
			//me->Yell(sMessage, 0);
    return false;
}
...
Unit* CombatBotBaseAI::SelectHealTarget(float selfHealPercent, float groupHealPercent) const
{
    ...
    if (Group* pGroup = me->GetGroup())
    {
        ...
        //pTarget = HealTarget[urand(0, HealTarget.size() - 1)];
       // healthPercent = pTarget->GetHealthPercent();
...
bool CombatBotBaseAI::IsValidHostileTarget(Unit const* pTarget) const
{
    ...
    return //(!pTarget->HasAura(8994) || !pTarget->HasAura(27565)) && //停止攻击被放逐目标
        ...
        for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* pMember = itr->getSource())
            {
                if (pMember->IsAlive() &&
                    ...
                    BuffTarget.push_back(pMember);
                    //return pMember;
            }
        }
        if (!BuffTarget.empty())
        {
            Player* pMember = BuffTarget[urand(0, BuffTarget.size() - 1)];
            return pMember;
        }
    }
...
Player* CombatBotBaseAI::SelectFearWardTarget(SpellEntry const* pSpellEntry) const
{
	Group* pGroup = me->GetGroup();
	if (pGroup)
	{
		for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
		{
			if (Player* pMember = itr->getSource())
			{
				...
				if (pMember->IsAlive() &&
					...
					IsValidBuffTarget(pMember, pSpellEntry) &&
					me->IsWithinLOSInMap(pMember) &&
					me->IsWithinDist(pMember, 30.0f) && pMember->GetClass() == CLASS_WARRIOR
					)
					return pMember;
			}
		}
...
Player* CombatBotBaseAI::SelectDispelTarget(SpellEntry const* pSpellEntry) const
{
    ...
    if (pGroup)
    {
        ...
        std::vector<Player*> DispelTarget;
        for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* pMember = itr->getSource())
            {
                ...
                if (pMember->IsAlive() &&
                    ...
                    DispelTarget.push_back(pMember);
                    //return pMember;
            }
        }
        if (!DispelTarget.empty())
        {
            Player* pMember = DispelTarget[urand(0, DispelTarget.size() - 1)];
            return pMember;
        }
    }
...
bool CombatBotBaseAI::CorpseNotResurrect(Player* Corpse) const
{
	Group* pGroup = Corpse->GetGroup();
	for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
	{
		if (Player* pResurrer = itr->getSource())
		{
			// Skip self.
			if (pResurrer == me)
				continue;

			if (pResurrer->GetSelectedPlayer() == Corpse && 
				(pResurrer->GetClass() == CLASS_PALADIN ||
				pResurrer->GetClass() == CLASS_PRIEST))
				return false;
		}
	}
	return true;
...
void CombatBotBaseAI::EquipRandomGearInEmptySlots()
{
    ...
    for (auto const& itr : sObjectMgr.GetItemPrototypeMap())
    {
        ...
        //if ((pProto->ItemLevel + sWorld.getConfig(CONFIG_UINT32_PARTY_BOT_RANDOM_GEAR_LEVEL_DIFFERENCE)) < me->GetLevel())//DMT
