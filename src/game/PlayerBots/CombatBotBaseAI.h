...
public:
	...
	Player* SelectFearWardTarget(SpellEntry const* pSpellEntry) const;
	Player* SelectDispelTarget(SpellEntry const* pSpellEntry) const;
	bool CorpseNotResurrect(Player* Corpse) const;
    ...
    SpellEntry const* SelectBestBlessingSpell(Unit const* pTarget) const;
    ...
    union
    {
        ...
        struct
        {
			...
			SpellEntry const* pTranquilizingShot; //19801
        ...
        struct
        {
            SpellEntry const* pPowerWordFortitude;	//’Ê—‘»Õ
            ...
            SpellEntry const* pFade;				//Ω•“˛
            SpellEntry const* pShackleUndead;
			SpellEntry const* pFearWard;	//∑¿ø÷Ω·ΩÁ ∞´»Àƒ¡ 19337 6346
        } priest;
        struct
        {
        ...
        struct
        {
			...
			SpellEntry const* pRemoveCurse;		//Ω‚≥˝◊Á÷‰
            // Cat
