...
void PartyBotAI::AddPremadeGearForClass()
{
	...
	for (const auto& itr : sObjectMgr.GetPlayerPremadeGearTemplates())
	{
		if (itr.second.requiredClass == me->GetClass() &&
			(itr.second.role == m_role || itr.second.role == ROLE_INVALID) &&
			itr.second.name.find("phase") != std::string::npos)
...
//<<<<<<< HEAD
//    me->GetMotionMaster()->MoveDistance(pTarget, 25.0f); //15
//=======
...
bool PartyBotAI::AttackStart(Unit* pVictim)
{
    ...
    if (me->Attack(pVictim, true))
    {
        ...
        me->GetMotionMaster()->MoveChase(pVictim, 3.0f, m_role == ROLE_MELEE_DPS ? 3.0f : 0.0f);
        return true;
    }
	/*else
	{
		...
		{		
			if (me->GetClass() == CLASS_MAGE ||
				...
				me->GetClass() == CLASS_HUNTER)
			{
				pAI->m_updateTimer.Reset(8000);	//远程输出8秒后开始
			}
			else if (pAI->m_role == ROLE_MELEE_DPS &&
					(me->GetClass() == CLASS_ROGUE ||
					me->GetClass() == CLASS_DRUID))
			{
				pAI->m_updateTimer.Reset(5000);	//近战5秒后上
			}
			else if (pAI->m_role == ROLE_HEALER)
			{
				pAI->m_updateTimer.Reset(5000);//治疗5秒后开始
			}
		}
	}*/
...
Player* PartyBotAI::SelectResurrectionTarget() const
{
    ...
    {
        std::vector<Player*> NeedResurrectionTarget;
        for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* pMember = itr->getSource())
            {
                ...
                if ((pMember->GetDeathState() == CORPSE || pMember->GetDeathState() == DEAD)
                    && me->IsWithinDist(pMember, 30.0f) && CorpseNotResurrect(pMember)
                    && me->IsWithinLOSInMap(pMember))
                    NeedResurrectionTarget.push_back(pMember);
                //return pMember;
            }
        }

        if (!NeedResurrectionTarget.empty())
        {
            Player* pMember = NeedResurrectionTarget[urand(0, NeedResurrectionTarget.size() - 1)];
            return pMember;
        }
    }
    return nullptr;
...
void PartyBotAI::UpdateAI(uint32 const diff)
{
    ...
    if (!m_initialized)
    {
		...
		else
		{
            ...
            AutoEquipGear(2);//(sWorld.getConfig(CONFIG_UINT32_PARTY_BOT_AUTO_EQUIP));DMT
    ...
    if (!me->IsInCombat())
    {
        ...
        if (tooFarAway || onDifferentTransports)
        {
            ...
            else // if (onDifferentTransports)
            {
                ...
                if (Transport* pMyTransport = me->GetTransport())
                {
                    sendHeartbeat = true;
                ...
                if (Transport* pHisTransport = pLeader->GetTransport())
                {
                    sendHeartbeat = true;
                    me->Relocate(pLeader->GetPositionX(), pLeader->GetPositionY(), pLeader->GetPositionZ());
        ...
        if (pVictim && pVictim->GetVictim() != me)
        {
            for (const auto& pSpellEntry : spellListTaunt)
            {
                if (CanTryToCastSpell(pVictim, pSpellEntry))
                {
					...
					{
						me->Yell((sPlayerBotMgr.GenerateBotSpeech("taunt").c_str()), 0);
						return;
					}
                }
            }
...
void PartyBotAI::UpdateInCombatAI_Hunter()
{
    ...
    if (Unit* pVictim = me->GetVictim())
    {
        ...
        if (me->GetMapId()== 409 && m_spells.hunter.pTranquilizingShot &&
...
void PartyBotAI::UpdateOutOfCombatAI_Priest()
{
	...
	{
		if (Player* pTarget = SelectFearWardTarget(m_spells.priest.pFearWard))
		{
...
void PartyBotAI::UpdateInCombatAI_Priest()
{
    ...
    if (m_role == ROLE_HEALER)
    {
		...
		//Fear Ward for TANK
		//if (me->GetRace() == RACE_DWARF && m_spells.priest.pFearWard)
		if (m_spells.priest.pFearWard)
		{
			if (Player* pTarget = SelectFearWardTarget(m_spells.priest.pFearWard))
			{
