...
void PlayerBotMgr::Load()
{
	...
	// 4-1 LoadbotspeechFromDB
	sLog.outString(">> [PlayerBotMgr] Loading Bots Speech List...");
	result = (CharacterDatabase.PQuery(
		"SELECT id, name, text, type"
		" FROM ai_playerbot_speech"));
	if (!result)
		sLog.outString("Table `ai_playerbot_speech` is empty.");
	else
	{
		do
		{
			fields = result->Fetch();
			std::string word = fields[2].GetString();
			std::string name = fields[1].GetString();
			ai_playbot_speech[name].push_back(word);

		} while (result->NextRow());
		delete result;
		sLog.outString("ai_speech Loaded");
	}

	sLog.outString(">> [PlayerBotMgr] Loading Bots Name List...");
	result = (CharacterDatabase.PQuery(
		"SELECT name, gender"
		" FROM ai_playerbot_names"));
	if (!result)
		sLog.outString("Table `ai_playerbot_names` is empty.");
	else
	{
		do
		{
...
std::string PlayerBotMgr::GenerateBotSpeech(std::string name)
{
	std::vector<std::string>& list0 = ai_playbot_speech[name];
...
std::string PlayerBotMgr::GenerateBotName(uint32 gender)
{
	...
	if (list0.empty())
		return sObjectMgr.GeneratePetName(1863);
...
void PlayerBotMgr::OnBotLogin(PlayerBotEntry *e)
{
    ...
    if (confDebug)
        sLog.outString("[PlayerBot][Login] GUID:%u Acc:%u '%s' LEVEL: %u", e->playerGUID, e->accountId, e->accountId, e->ai->me->GetName(), e->ai->me->GetLevel());
}
void PlayerBotMgr::OnBotLogout(PlayerBotEntry *e)
{
    e->state = PB_STATE_OFFLINE;
    if (confDebug)
        sLog.outString("[PlayerBot][Logout] GUID:%u Acc:%u '%s' LEVEL: %u" , e->playerGUID, e->accountId, e->ai->me->GetName(), e->ai->me->GetLevel());
}
...
bool PlayerBotMgr::AddBot(PlayerBotAI* ai, uint32 guid)
{
	// Find a correct accountid ?
	PlayerBotEntry* e = new PlayerBotEntry();
	e->ai = ai;
	e->accountId = sObjectMgr.GetPlayerAccountIdByGUID(guid);;
	e->playerGUID = guid;
	e->customBot = true;
	ai->botEntry = e;
	m_bots[e->playerGUID] = e;
	AddBot(e->playerGUID, false);
	if (!sObjectMgr.GetPlayerNameByGUID(e->playerGUID, e->name))
		e->name = "<Unknown>";
	return true;
}
...
bool ChatHandler::HandleBotAddCommand(char* args)
{
    ...
    if (!args)
    {
        SendSysMessage("Incorrect syntax. Expected Character Name.");
	...
	CharBotAI* ai = new CharBotAI();

    if (!guid || !sPlayerBotMgr.AddBot(ai,guid))
    {
...
bool ChatHandler::HandlePartyBotAddCommand(char* args)
{
    ...
    uint8 botClass = 0;
    //uint32 botLevel = sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL); //DMT
    uint32 botLevel = pPlayer->GetLevel() < 50 ? pPlayer->GetLevel() / 10 * 10 + 9 : sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL); //DMT

    sLog.outString("[PartyBot][ReadyLogin] Level:%u", botLevel);
	...
	else if (option == "warrior")
	{
		...
		botRole = PickRandomValue(ROLE_TANK, ROLE_MELEE_DPS);
	}
	else if (option == "paladin" && pPlayer->GetTeam() == ALLIANCE)
	{
		botClass = CLASS_PALADIN;
		botRole = PickRandomValue(ROLE_TANK, ROLE_MELEE_DPS, ROLE_HEALER);
	}
	...
	else if (option == "priest")
	{
		...
		std::vector<uint32> pRole = {  };
		botRole = PickRandomValue(ROLE_HEALER, ROLE_RANGE_DPS);
	}
	...
	else if (option == "druid")
	{
		...
		botRole = PickRandomValue(ROLE_TANK, ROLE_RANGE_DPS, ROLE_HEALER);
	}
...
bool ChatHandler::HandlePartyBotUseGObjectCommand(char* args)
{
    ...
    ChatHandler(pPlayer).HandleGameObjectSelectCommand(args);
...
bool ChatHandler::HandlePartyBotSubgroupCommand(char* args)
{
    //int32 Subgroup;
    //if (!ExtractOptInt32(&args, Subgroup, 1))
    //    return false;
    ...
    if (pTarget && pTarget != pPlayer)
    {
        if (ok = HandlePartyBotComeToMeHelper(pTarget, pPlayer))
        {
            sprintf(sMessage, "I am in Subgroup %d", pTarget->GetSubGroup());
            pTarget->Yell(sMessage, 0);
        }
        else
        {
            PSendSysMessage("%s is not a party bot or it cannot move.", pTarget->GetName());
    ...
    else if (Group* pGroup = pPlayer->GetGroup())
    {
        ...
        for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* pMember = itr->getSource())
            {
                ...
                sprintf(sMessage, "I am in Subgroup %d", pMember->GetSubGroup());
                pMember->Yell(sMessage, 0);
            }
        }
